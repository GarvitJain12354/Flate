...
import { rentaggreement } from "../../store/Action/User";
import { saveAs } from "file-saver"; // You need this to download the file
  ...
  <Document>
    <Page style={styles.page}>
      ...
      <Text style={styles.paragraph}>
        ...
        {propertyDetails?.ownerAddress} (hereinafter jointly and severally
        called the "LANDLORD").
      </Text>
...
const GeneratedAgreement = ({ handleCancel, amount, data }) => {
  ...
  const handleDownload = async () => {
    try {
      ...
      formData.append("email", agreementTerms.email);
      formData.append("file", blob, "rentagreement.pdf");
      dispatch(rentaggreement(formData));
      btn.current.click();
      handleCancel();
    } catch (error) {
      console.log("====================================");
  ...
  return (
    <>
      <RazorpayPayment
        classn={"w-full  text-white py-2 rounded-lg font-bold  bg-primary"}
        text={"Pay Now"}
        amount={amount}
