...
const { sendmail, sendEmailPdf } = require("../nodemailer/nodemailer");
...
exports.getListingMatchings = CatchAsyncErrors(async (req, res, next) => {
  ...
  const matchedUsers = users.map((user) => {
    ...
    const userPreferences = (user.prefrence || []).map((p) => (p?.prefrence || "").toLowerCase());
    const listingUserPreferences = (listing.user.prefrence || []).map((p) => (p?.prefrence || "").toLowerCase());
    
    let preferenceMatchScore = 0;
    preferenceValues.forEach((preference) => {
      if (
        userPreferences.includes(preference) &&
        listingUserPreferences.includes(preference)
      ) {
...
exports.generateRentAgreement = CatchAsyncErrors(async(req,res,next)=>{
  try {
    if (!req.files) {
      return res.status(400).send('No file uploaded.');
    }
    const uploadedFile = req.files.file;
    await sendEmailPdf(uploadedFile,req.body.email);
    res.status(200).json({
      success:true,
